...
public class DiffAST {
	...
	public static List<MethodInfo> diffDir(final String ntag,
		...
		// 如果后面不加参数，则直接对比一个不存在的文件夹，等于取全量
		String baseDir = "";
		if ((!System.getProperties().containsKey("baseDir")) && !(System.getProperties().containsKey("repo"))){
			baseDir = "xxxxxxxxx";
		}
		baseDir = otag;
	...
	public static List<MethodInfo> diffBaseDir(final String baseDir) {// src1是整个工程中有变更的文件,src2是历史版本全量文件,都是相对路径,例如在当前工作空间下生成tag1和tag2
		final String pwd = new File(System.getProperty("user.dir"))
				.getAbsolutePath();// 同级目录
		final String parent = new File(System.getProperty("user.dir")).getParent();
		final String tag1Path = pwd;
